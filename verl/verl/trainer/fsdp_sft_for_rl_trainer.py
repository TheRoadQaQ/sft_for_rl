    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                # gather entropy_rmpad from all ulysses ranks
                entropy = gather_outpus_and_unpad(entropy_rmpad, gather_dim=0, unpad_dim=0, padding_size=pad_size)
                
                # pad entropy back to original shape
                full_entropy = pad_input(hidden_states=entropy.unsqueeze(-1), indices=indices, batch=batch_size, seqlen=seqlen)
                full_entropy = full_entropy.squeeze(-1)[:, :-1]  # 移除最后一个token的熵
                full_entropy = full_entropy.reshape(-1)
                masked_entropy = full_entropy * loss_mask.to(full_entropy.device)
