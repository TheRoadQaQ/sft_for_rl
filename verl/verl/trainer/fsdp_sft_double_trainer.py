...
class FSDPSFTWithKLTrainer:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        """
        Theory:
        Minimizes KL(Q* || Student).
        To prevent numerical collapse in downstream RL (due to extremely sharp logits),
        we apply Reference Smoothing BEFORE boosting the label signal.
        
        Q*(y) \propto ( (1-eps)*P_ref + eps*Uniform ) * exp(Beta * r(y))
        """
        ...
        # === [核心参数: Logit 增益 & 平滑系数] ===
        # TARGET_GAIN = 10.0:d 
        #   提供强信号，确保难样本(p_ref=0.001)的目标概率能达到 >0.95，解决欠拟合。
        # SMOOTH_LAMBDA = 0.01: 
        #   数值保险丝。防止 p_ref -> 0 时产生 -inf 的 Logit。
        #   这保证了 RL 阶段的 KL 散度不会因为分母过小而爆炸。
        TARGET_GAIN = 30.0
        SMOOTH_LAMBDA = 0.01

        # Ref Model (必须有，用于提供 Base 信念)
        ref_model = getattr(self, "fsdp_ref_model", getattr(self, "ref_fsdp_model", None))
        
        # Standard CE Loss (用于验证)
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                if do_backward and ref_model is not None:
                    # === [Training Phase] KL(Q* || Student) ===
                    with torch.no_grad():
                        # A. 获取 Ref Logits (基准分布)
                        ...
                        # === [Critical Fix: Reference Smoothing] ===
                        # 1. 转为概率分布
                        ref_probs = F.softmax(ref_logits, dim=-1)
                        # 2. 混合均匀分布 (解毒步骤：消除 0 概率)
                        mixed_ref_probs = (1 - SMOOTH_LAMBDA) * ref_probs + SMOOTH_LAMBDA * (1.0 / vocab_size)
                        # 3. 转回 Log 域作为 Base Logit
                        # 加上 1e-10 防止 log(0)
                        ideal_logits = torch.log(mixed_ref_probs + 1e-10)
                        
                        # B. 构造 Q* (Twisted Distribution)
                        # 在 Label 位置注入能量 Beta
                        # Q*_logit = Smoothed_Ref_Logit + GAIN * OneHot
                        gain_tensor = torch.tensor(TARGET_GAIN, device=ideal_logits.device, dtype=ideal_logits.dtype)
                        gain_tensor = gain_tensor.expand(labels_flat.size(0), 1)

                        ideal_logits.scatter_add_(1, labels_flat.unsqueeze(1), gain_tensor)
                        
                        # 归一化得到 Q* (Boltzmann Policy)
                        ideal_probs = F.softmax(ideal_logits, dim=-1)

                    # C. 计算 KL 散度部分的 CrossEntropy: H(Q*, P_student)
                    student_log_probs = F.log_softmax(student_logits, dim=-1)
                    
                    # Loss = - sum( Q* * log(P_student) )
                    loss_per_token = -torch.sum(ideal_probs * student_log_probs, dim=-1)
                ...
                # Monitoring
                with torch.no_grad():
                    log_probs_mon = F.log_softmax(student_logits, dim=-1)
                    probs_mon = torch.exp(log_probs_mon)
                    entropy_per_token = -torch.sum(probs_mon * log_probs_mon, dim=-1)
                ...
                if do_backward and ref_model is not None:
                    ...
                    with torch.no_grad():
                        ...
                        # === [Critical Fix: Reference Smoothing (Local)] ===
                        ...
                        ideal_logits_local = torch.log(mixed_ref_probs_local + 1e-10)
                        
                        # Boost
                        gain_tensor = torch.tensor(TARGET_GAIN, device=ideal_logits_local.device, dtype=ideal_logits_local.dtype)
                        gain_tensor = gain_tensor.expand(labels_local.size(0), 1)

                        ideal_logits_local.scatter_add_(1, labels_local.unsqueeze(1), gain_tensor)
                        ideal_probs_local = F.softmax(ideal_logits_local, dim=-1)
                    
                    student_log_probs_local = F.log_softmax(student_logits_local, dim=-1)
                    loss_local = -torch.sum(ideal_probs_local * student_log_probs_local, dim=-1)
                else:
                ...
                log_probs_local = F.log_softmax(student_logits_local, dim=-1)
                probs_local = torch.exp(log_probs_local)
                entropy_local = -torch.sum(probs_local * log_probs_local, dim=-1)
                entropy_gathered = gather_outpus_and_unpad(entropy_local, gather_dim=0, unpad_dim=0, padding_size=pad_size)
                ...
                masked_entropy = restore_shape(entropy_gathered) * loss_mask
            ...
            avg_entropy = torch.sum(masked_entropy) / denom * dp_size
    ...
    def training_step(self, batch: TensorDict):
        ...
        # [修改点] 变量精简，只统计 Total Loss 和 Entropy
        step_loss = 0.0
        step_entropy = 0.0
        ...
        for micro_batch in micro_batches:
            ...
            step_entropy += entropy.item() / n_micro_batches
        ...
        # [修改点] 更新 Metrics，移除了不再单独返回的 CE/KL
        # train/loss 现在即为包含 Inverse-CAR 正则的总 Loss
        metrics = {
    ...
    def validation_step(self, batch: TensorDict):
        ...
        with torch.no_grad():
            # [修改点] 验证阶段 do_backward=False
            # 根据 _compute_loss_and_backward 逻辑，此时返回的 loss 就是纯 CE Loss
            ce_loss, entropy = self._compute_loss_and_backward(batch, do_backward=False)
        ...
        # 返回纯 CE loss 和 Entropy 供验证集评估
        return ce_loss, entropy
            ...
            for data in tqdm(
                ...
                if is_last_step or (self.config.trainer.test_freq > 0 and is_valid_step):
                    ...
                    for val_data in self.val_dataloader:
                        ...
                        val_ce_loss, val_entropy = self.validation_step(val_data)
